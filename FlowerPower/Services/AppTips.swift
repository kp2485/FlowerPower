...
//  FlowerPower
//
//  The four things the introduction cannot say.
//
...
//  for anything specific: a list of what each of the three tabs does, read
...
//
//  Four, and not one more. Every tip is an interruption, and the fastest way
//  to have all of them ignored is to write a fifth about something the player
...
enum AppTips {
    ...
    /// player who explores all three tabs in their first minute would otherwise
    /// get every one of these in that minute, having just read three pages of
...
struct PhotographTip: Tip {
    ...
    var message: Text? {
        Text("Photographs are the only forage your bees have. Anything actually in bloom will do, and the more families you find them across, the longer the colony eats.")
    }
...
///
/// The gear is in all three tabs' navigation bars (`SettingsButton`), so
